...
const Navbar = ({ currentPage, onPageChange }) => {
  ...
  const navItems = useMemo(() => {
    const items = [
      ...
      { id: 'direct-messages', label: 'Messages', icon: '💬' },
    ...
    <nav className="bg-gray-900/95 backdrop-blur-md border-b border-blue-500/30 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 gap-4">
          ...
          {/* Logo & Welcome */}
          <div className="flex-1 flex items-center justify-between md:justify-start gap-6">
            <div className="flex items-center gap-6">
              ...
              <span className="font-semibold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent transition-all duration-300 hover:brightness-110 cursor-default hidden sm:block">
                  ...
                  <div className="flex items-baseline space-x-4">
                    {navItems.map((item) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-md text-sm font-medium flex items-center gap-2 ${currentPage === item.id
            ...
            <>
              ...
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => handleNavClick(item.id)}
                  className={`w-full text-left px-3 py-2 rounded-md text-base font-medium flex items-center gap-3 ${currentPage === item.id ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-blue-600/50'
                    }`}
                >
                  <span>{item.icon}</span>
                  {item.label}
