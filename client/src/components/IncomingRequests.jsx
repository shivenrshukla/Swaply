...
import MatchConfirmationModal from './MatchConfirmationModel';
...
const IncomingRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      console.log('Raw request data:', data.requests);
      if (data.requests && data.requests.length > 0) {
        console.log('First request structure:', data.requests[0]);
        console.log('skillOffered:', data.requests[0].skillOffered);
        console.log('skillRequested:', data.requests[0].skillRequested);
      }
  ...
  const getSkillName = (skillObj) => {
    ...
    if (skillObj.skillName) return skillObj.skillName;
    if (skillObj.name) return skillObj.name;
              ...
              <div key={request._id} className="bg-gradient-to-br from-gray-800 to-blue-gray-900 border border-blue-700 rounded-2xl p-6">
                <div className="flex flex-col md:flex-row md:items-start gap-6">
                  <div className="flex items-center gap-4 md:w-1/3">
                    <img
                      src={request.requester.profilePhoto || `https://placehold.co/64x64/1e293b/e2e8f0?text=${request.requester.name.charAt(0)}`}
                  ...
                  <div className="flex-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="bg-green-600/20 p-3 rounded-lg border border-green-500/30">
                        ...
                        <p className="text-green-300 font-medium">{getSkillName(request.skillOffered)}</p>
                      </div>
                      <div className="bg-blue-600/20 p-3 rounded-lg border border-blue-500/30">
                        <p className="text-xs text-blue-200 mb-1">THEY WANT</p>
                        <p className="text-blue-300 font-medium">{getSkillName(request.skillRequested)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        disabled={actionLoading[request._id]}
                        className="bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 disabled:opacity-50 text-white font-semibold px-4 py-2 rounded-lg"
                      >
